...
for /f "tokens=* delims=" %%a in ('dir /b "%dirpath%\Emulators\ROMS\N64\*.*"') do (
    set "oldname=%%a"
    set "extension=%%~xa"
    for /f usebackq^ tokens^=1^ delims^=^(^" %%f in ('"%%a"') do (
        ...
        set newname=!newname:%underscore%=%space%!
            for /f "tokens=* delims= " %%d in ('echo "!newname!"') do (
                set newname=%%~d
                move /Y "%dirpath%\Emulators\ROMS\N64\!oldname!" "%dirpath%\Emulators\ROMS\N64\!newname:%underscore%=%space%!!extension!"
                endlocal
            )
        )
    )   
...
for /f "tokens=* delims=" %%A in ('dir /b /s "%dirpath%\Steam_Shortcuts\N64\*.bat"') do (
    del /F /Q "%%A"
)
cls

::Create ROM List
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\N64\*.n64"') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\N64\*.v64"') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\N64\*.z64"') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
cls
