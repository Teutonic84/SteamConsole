...
for /f "tokens=* delims=" %%a in ('dir /b "%dirpath%\Emulators\ROMS\NES\*.*"') do (
    set "oldname=%%a"
    set "extension=%%~xa"
    for /f usebackq^ tokens^=1^ delims^=^(^" %%f in ('"%%a"') do (
        ...
        set newname=!newname:%underscore%=%space%!
            for /f "tokens=* delims= " %%d in ('echo "!newname!"') do (
                set newname=%%~d
                move /Y "%dirpath%\Emulators\ROMS\NES\!oldname!" "%dirpath%\Emulators\ROMS\NES\!newname:%underscore%=%space%!!extension!"
                endlocal
            )
        )
    )   
...
for /f "tokens=* delims=" %%A in ('dir /b /s "%dirpath%\Steam_Shortcuts\NES\*.bat"') do (
    del /F /Q "%%A"
)

::Create ROM List
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\NES\*.zip"') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\NES\*.nes"') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
cls
