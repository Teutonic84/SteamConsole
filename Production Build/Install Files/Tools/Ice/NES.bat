...
::| Start ROM Renamer |
::=====================
for /f "tokens=* delims=" %%a in ('dir /b "%dirpath%\Emulators\ROMS\NES\*.*" 2^>NUL') do (
    call :check "%%a"
)
goto renamer
    ...
    for /f "usebackq tokens=1 delims=(" %%f in ('"!newname!"') do (
        ...
        for /f usebackq^ tokens^=1^ delims^=^[^" %%b in ('"%%f"') do (
            ...
            for /f "tokens=* delims= " %%d in ('echo "!newname!"') do (
                set "newname=%%~d%"
                move /Y "%dirpath%\Emulators\ROMS\NES\!oldname!" "%dirpath%\Emulators\ROMS\NES\!newname!!extension!" 2>NUL 1>NUL
                endlocal
            )
        )
    )
...
for /f "tokens=* delims=" %%A in ('dir /b /s "%dirpath%\Steam_Shortcuts\NES\*.bat" 2^>NUL') do (
    del /F /Q "%%A"
)

::Create ROM List
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\NES\*.zip" 2^>NUL') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\NES\*.nes" 2^>NUL') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
::cls
...
for /f "tokens=* delims=" %%M in ('Type "%romlist%" 2^>NUL') do (
    ...
    set str=%%L
    set str=!str:%old4%=%%M!
    echo !str!>>"%dirpath%\Steam_Shortcuts\NES\%%M.bat"
    )
)
...
For %%# in ("%dirpath%\Steam_Shortcuts\NES\*.bat") Do (
    Set "File=%%~nx#"
