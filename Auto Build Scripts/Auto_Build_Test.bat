...
robocopy "..\Test Build" "%bpath%\Test Build" /E
COPY /y ".\SteamConsole_uninstaller.exe" "%bpath%\Test Build"
ECHO Done
ECHO.
PING localhost -n 5 >NUL
...
7zg a -y "%bpath%\Updater_v%updaterv%.7z" "%bpath%\Test Build\Install Files\Scripts\Updater"
ECHO Done
ECHO.
PING localhost -n 5 >NUL
...
"%bpath%\Test Build\Changelog.rtf"
ECHO Done
ECHO.
PING localhost -n 5 >NUL

ECHO Zipping Updated SteamConsole Files
MOVE /Y "%bpath%\Test Build\SteamConsole_Setup.nsi" "%bpath%\SteamConsole_Setup.nsi" >NUL
7zg a -y "%bpath%\Test Build\SteamConsole_v%scversion%_x64.7z" "%bpath%\Test Build\*"
ECHO Done
ECHO.

ECHO Moving "SteamConsole_v%scversion%_x64.7z" To FTP Folder On \\Server.
MOVE /Y "%bpath%\Test Build\SteamConsole_v%scversion%_x64.7z" "\\server\c$\ftp_root\steamconsole\SteamConsole_v%scversion%_x64.7z"
ECHO Done
ECHO.
PING localhost -n 5 >NUL

ECHO Prepping Files For SteamConsole NSIS EXE Build.
MOVE /Y "%bpath%\SteamConsole_Setup.nsi" "%bpath%\Test Build\SteamConsole_Setup.nsi"
7zg a -y "%bpath%\Test Build\SteamConsole.7z" "%bpath%\Test Build\Install Files\*"
ECHO Done
ECHO.
PING localhost -n 5 >NUL

ECHO Removing "..\Install Files" Folder.
RMDIR /S /Q "%bpath%\Test Build\Install Files"
ECHO Done
ECHO.
PING localhost -n 5 >NUL

ECHO Edit The File "SteamConsole_Setup.nsi" With SteamConsole Version Number %scversion% Before Continuing.
notepad "%bpath%\Test Build\SteamConsole_Setup.nsi"
ECHO Done
ECHO.
PING localhost -n 5 >NUL

ECHO Building "SteamConsole_Setup.nsi"
"C:\Program Files (x86)\NSIS\makensis.exe" /X"SetCompressor /FINAL lzma" "%bpath%\Test Build\SteamConsole_Setup.nsi"
RENAME "%bpath%\Test Build\SteamConsole_Setup.exe" "SteamConsole_v%scversion%_x64.exe"
ECHO Done
ECHO.
PING localhost -n 5 >NUL

ECHO Moving "SteamConsole_v%scversion%.exe" To "Builds" Root Folder.
MOVE /Y "%bpath%\Test Build\SteamConsole_v%scversion%_x64.exe" "%userprofile%\Desktop\Builds\SteamConsole_v%scversion%_x64.exe"
ECHO Done
ECHO.
PING localhost -n 5 >NUL
