    ...
    for /f usebackq^ tokens^=1^ delims^=^(^" %%f in ('"%%a"') do (
        ...
        set newname=!newname:%underscore%=%space%!
            for /f "tokens=* delims= " %%d in ('echo "!newname!"') do (
                set newname=%%~d
                move /Y "%dirpath%\Emulators\ROMS\PS1\!oldname!" "%dirpath%\Emulators\ROMS\PS1\!newname:%underscore%=%space%!!extension!"
                endlocal
            )
        )
    )   
...
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\PS1\*.bin"') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
cls
...
for /f "tokens=* delims=" %%M in ('Type "%romlist%"') do (
    ...
    set str=%%L
    set str=!str:%old4%=%%M"!
    echo !str!>>"%dirpath%\Steam_Shortcuts\PS1\%%M.bat"
    )
)
