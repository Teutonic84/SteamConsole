...
::| Start ROM Renamer |
::=====================
for /f "tokens=* delims=" %%a in ('dir /b "%dirpath%\Emulators\ROMS\PS2\*.*" 2^>NUL') do (
    call :check "%%a"
)
goto renamer
    ...
    for /f "usebackq tokens=1 delims=(" %%f in ('"!newname!"') do (
        ...
        for /f usebackq^ tokens^=1^ delims^=^[^" %%b in ('"%%f"') do (
            ...
            for /f "tokens=* delims= " %%d in ('echo "!newname!"') do (
                set "newname=%%~d%disc%"
                move /Y "%dirpath%\Emulators\ROMS\PS2\!oldname!" "%dirpath%\Emulators\ROMS\PS2\!newname!!extension!" 2>NUL 1>NUL
                endlocal
            )
        )
    )
...
for /f "tokens=* delims=" %%A in ('dir /b /s "%dirpath%\Steam_Shortcuts\PS2\*.bat" 2^>NUL') do (
    del /F /Q "%%A"
)
::cls

::Create ROM List
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\PS2\*.iso" 2^>NUL') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\PS2\*.bin" 2^>NUL') do (
    echo %%F>>"%dirpath%\Tools\Ice\ROMS.txt"
)
::cls
