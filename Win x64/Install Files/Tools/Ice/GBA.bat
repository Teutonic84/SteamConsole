...
for /f "tokens=* delims=" %%a in ('dir /b "%dirpath%\Emulators\ROMS\GBA\*.*"') do (
    set "oldname=%%a"
    set "extension=%%~xa"
    for /f usebackq^ tokens^=1^ delims^=^(^" %%f in ('"%%a"') do (
        ...
        set newname=!newname:%underscore%=%space%!
            for /f "tokens=* delims= " %%d in ('echo "!newname!"') do (
                set newname=%%~d
                move /Y "%dirpath%\Emulators\ROMS\GBA\!oldname!" "%dirpath%\Emulators\ROMS\GBA\!newname:%underscore%=%space%!!extension!"
                endlocal
            )
        )
    )   
...
for /f "tokens=* delims=" %%A in ('dir /b /s "%dirpath%\Steam_Shortcuts\GBA\*.bat"') do (
    del /F /Q "%%A"
)
cls

::Create ROM List
for /f "tokens=* delims=" %%F in ('dir /b "%dirpath%\Emulators\ROMS\GBA\*.gba"') do (
