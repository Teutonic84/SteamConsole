...
:app_path
FOR /f "usebackq tokens=*" %%a IN ("D:\GitHub\SteamConsole\Test Build\Install Files\Config\general_settings.ini") DO (
...
FOR /f "skip=1 tokens=*" %%a IN ('Reg query "%key%" /s /f "%browser%"') DO if not defined line set "line=%%a" 2>NUL 1>NUL
